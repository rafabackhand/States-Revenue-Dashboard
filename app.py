...
app.layout = html.Div(
    ...
    children=[
        ...
        # --- Section 4: Fiscal capacity (Acemoglu-Johnson-Robinson) ---
        html.H3(
            "State fiscal capacity — the Acemoglu-Johnson-Robinson lens",
            style={"marginTop": "28px", "borderBottom": "1px solid #EEE",
                   "paddingBottom": "6px"}),

        html.Div(
            style={"backgroundColor": "#FFF8E1", "padding": "12px 16px",
                   "borderRadius": "6px", "marginBottom": "16px",
                   "borderLeft": "4px solid #F4A340",
                   "fontSize": "13px", "color": "#333",
                   "lineHeight": "1.55"},
            children=[
                html.B("Why this matters. "),
                "Acemoglu, Johnson & Robinson won the ",
                html.B("2024 Nobel Prize in Economics"),
                " for showing that ",
                html.I("institutional quality"),
                " — in particular a state's ability to tax its own citizens "
                "and deploy revenue productively — is the deepest driver of "
                "long-run prosperity. States that depend heavily on external "
                "transfers tend to develop weaker institutions because "
                "domestic taxation is what forces accountability between "
                "government and citizens.",
                html.Br(),
                html.Br(),
                "Two observable dimensions of state capacity from our data:",
                html.Ul([
                    html.Li([
                        html.B("Tax effort "), "(horizontal axis): SOTR per "
                        "capita. Proxies tax-administration strength."]),
                    html.Li([
                        html.B("Fiscal autonomy "), "(vertical axis): own "
                        "revenue as % of total receipts. Higher = less "
                        "dependent on the Centre."]),
                ], style={"marginTop": "6px", "marginBottom": "6px"}),
                "Bubble size = total revenue (scale). Dashed lines are "
                "medians across states. Quadrants interpret accordingly:",
                html.Ul([
                    html.Li([html.B("Top-right: "),
                             "Strong capacity + autonomous (the "
                             "AJR-ideal)."]),
                    html.Li([html.B("Bottom-left: "),
                             "Weak capacity + dependent (institutional "
                             "catch-up needed)."]),
                    html.Li([html.B("Top-left: "),
                             "Small/rich states — autonomous with low "
                             "effort because they don't need much."]),
                    html.Li([html.B("Bottom-right: "),
                             "High tax effort but still dependent — "
                             "structurally large states like UP/Bihar "
                             "whose citizens are too poor for own tax to "
                             "dominate despite strong tax effort."]),
                ], style={"marginTop": "4px", "marginBottom": "2px"}),
            ]),

        dcc.Graph(id="capacity-quadrant"),

        html.Div(id="capacity-diagnosis",
                 style={"marginTop": "10px", "marginBottom": "28px",
                        "padding": "12px 16px",
                        "backgroundColor": "#F7F7F9", "borderRadius": "6px",
                        "fontSize": "13px", "color": "#333",
                        "lineHeight": "1.5"}),

        # --- Section 5: full rankings ---
...
@app.callback(
    Output("capacity-quadrant", "figure"),
    Output("capacity-diagnosis", "children"),
    Input("state-dropdown", "value"),
    Input("peer-dropdown", "value"),
    Input("period-radio", "value"),
)
def render_capacity_quadrant(state, peers, period):
    """AJR-inspired Fiscal Capacity Quadrant.

    X: SOTR per capita (tax-effort / administrative capacity).
    Y: Own revenue share (autonomy from central transfers).
    Median lines split states into four quadrants.
    """
    peers_set = {p for p in (peers or []) if p != state}
    wide = period_wide(period)

    sotr = wide["SOTR"]
    nontax = wide["Non-Tax Revenue"]
    total = wide["Total Revenue Receipts"]
    pop = wide["population"]

    x = (sotr * 1e7 / pop)                  # ₹ per person
    y = ((sotr + nontax) / total) * 100     # own revenue share, %
    size_raw = total.fillna(1)

    # Normalise bubble sizes to a readable 8–48 px range.
    s_min, s_max = size_raw.min(), size_raw.max()
    sizes = 8 + (size_raw - s_min) / max(s_max - s_min, 1) * 40
    ...
    colors = {"focus": C_PRIMARY, "peer": C_PEER_AVG, "other": "#BBBBBB"}
    cats = [cat(s) for s in wide.index]

    fig = go.Figure()
    # Plot each category separately so the legend is clean.
    for c in ["other", "peer", "focus"]:
        mask = [cc == c for cc in cats]
        if not any(mask):
            continue
        xs = x[mask]
        ys = y[mask]
        sz = sizes[mask]
        names = wide.index[mask]
        label = {"focus": f"{state} (focus)",
                 "peer": "Peer group",
                 "other": "Other states"}[c]
        fig.add_trace(go.Scatter(
            x=xs, y=ys, mode="markers+text" if c != "other" else "markers",
            marker=dict(size=sz, color=colors[c], opacity=0.85,
                        line=dict(width=1, color="white")),
            text=names if c != "other" else None,
            textposition="top center",
            textfont=dict(size=10, color="#333"),
            name=label,
            customdata=np.stack([total[mask], sotr[mask], nontax[mask]],
                                axis=-1),
            hovertemplate=(
                "<b>%{text}</b><br>"
                if c != "other" else
                "<b>%{customdata[0]}</b><br>"
            ).replace("customdata[0]", "text") +
            "Tax effort (SOTR / capita): ₹%{x:,.0f}<br>"
            "Fiscal autonomy: %{y:.1f}%<br>"
            "Total revenue: ₹%{customdata[0]:,.0f} Cr<br>"
            "<extra></extra>",
        ))

    # Re-add other-states hover with proper name labels.
    mask_other = [c == "other" for c in cats]
    if any(mask_other):
        # Remove the earlier 'other' trace we added and replace with a
        # correctly-configured one. Simpler: overwrite the first (other) trace.
        fig.data = tuple(t for t in fig.data
                         if t.name != "Other states")
        names_o = wide.index[mask_other]
        fig.add_trace(go.Scatter(
            x=x[mask_other], y=y[mask_other], mode="markers",
            marker=dict(size=sizes[mask_other], color="#BBBBBB",
                        opacity=0.6,
                        line=dict(width=1, color="white")),
            name="Other states",
            text=names_o,
    ...
    fig.add_hline(y=y_med, line_dash="dash", line_color="#999",
                  line_width=1)
    fig.add_vline(x=x_med, line_dash="dash", line_color="#999",
                  line_width=1)

    x_max, y_max = x.max() * 1.05, min(y.max() * 1.08, 100)
    x_min, y_min = 0, max(y.min() - 3, 0)

    fig.add_annotation(
        x=x_max, y=y_max, xref="x", yref="y",
        text="<b>Strong capacity<br>+ autonomous</b>",
        showarrow=False,
        font=dict(size=10, color="#2CA02C"), align="right",
        xanchor="right", yanchor="top",
    )
    fig.add_annotation(
        x=x_min, y=y_max, xref="x", yref="y",
        text="<b>Autonomous but<br>low tax effort</b>",
        showarrow=False,
    ...
    fig.add_annotation(
        ...
        text="<b>High effort<br>but dependent</b>",
        showarrow=False,
        font=dict(size=10, color="#888"), align="right",
        xanchor="right", yanchor="bottom",
    )
    fig.add_annotation(
        x=x_min, y=y_min, xref="x", yref="y",
        text="<b>Weak capacity<br>+ dependent</b>",
        showarrow=False,
        font=dict(size=10, color=C_PRIMARY), align="left",
        xanchor="left", yanchor="bottom",
    )

    fig.update_layout(
        xaxis=dict(title="Tax effort — SOTR per capita (₹/person) →"),
        yaxis=dict(title="Fiscal autonomy — own revenue as % of total ↑",
                   range=[0, min(100, y_max + 5)]),
        height=560,
    ...
    # Auto-generated diagnosis text for the focus state.
    fs_x, fs_y = x[state], y[state]
    x_rank = int(x.rank(ascending=False, method="min")[state])
    y_rank = int(y.rank(ascending=False, method="min")[state])

    if fs_x >= x_med and fs_y >= y_med:
        quadrant, tone = "Strong capacity + autonomous", "#2CA02C"
        interp = (f"{state} is in the AJR-ideal quadrant — high tax "
                  "administration strength and low dependence on central "
                  "transfers.")
    elif fs_x < x_med and fs_y >= y_med:
        quadrant, tone = "Autonomous but low tax effort", "#888"
        interp = (f"{state} is autonomous on paper but collects little "
                  "own tax per citizen. Often small / high-income / "
                  "resource-rich states land here.")
    elif fs_x >= x_med and fs_y < y_med:
        quadrant, tone = "High tax effort but dependent", "#888"
        interp = (f"{state} collects strongly on own taxes but central "
                  "transfers still dominate — typical of populous states "
                  "where citizens' average income is low, so even good "
                  "tax effort yields a small share of total revenue.")
    else:
        quadrant, tone = "Weak capacity + dependent", C_PRIMARY
        interp = (f"{state} sits in the bottom-left: both tax effort and "
                  "autonomy trail the median. The AJR view treats this "
                  "as a long-run institutional-capacity gap, not a "
                  "short-run revenue problem.")

    diagnosis = html.Div([
        html.Span("Diagnosis for ", style={"color": "#666"}),
        html.B(state),
        html.Span(f" ({PERIODS[period]['label']}): ",
                  style={"color": "#666"}),
        html.Span(quadrant,
                  style={"fontWeight": "700", "color": tone}),
        html.Br(),
        html.Span(
            f"Tax effort: ₹{fs_x:,.0f}/person "
            f"(rank {x_rank}/{len(wide)} — "
            f"{'above' if fs_x >= x_med else 'below'} median ₹{x_med:,.0f}). ",
            style={"color": "#444"}),
        html.Span(
            f"Fiscal autonomy: {fs_y:.1f}% "
            f"(rank {y_rank}/{len(wide)} — "
            f"{'above' if fs_y >= y_med else 'below'} median "
            f"{y_med:.1f}%). ",
            style={"color": "#444"}),
        html.Br(),
        html.Span(interp, style={"color": "#333", "fontStyle": "italic"}),
    ])
    return fig, diagnosis
